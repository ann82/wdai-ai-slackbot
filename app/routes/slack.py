...
async def slack_events(request: Request):
    ...
    try:
        ...
        logger.info(
            f"Received event type: {event_type}, event_ts: {event.get('ts')}, user: {event.get('user')}"
        )
        ...
        if is_limited:
            logger.warning(
                f"Rate limited user {user_id}: {reason}",
        ...
        if is_image_request and image_prompt:
            ...
            if image_url:
                ...
                if image_data:
                    logger.info(
                        f"Uploading image to Slack, size: {len(image_data)} bytes"
                    )
                        ...
                        if upload_success:
                            try:
                                slack_client.chat_delete(
                                    channel=channel, ts=processing_msg_ts
                                )
                            except Exception as delete_error:
                                logger.warning(
                                    f"Could not delete processing message: {delete_error}"
                                )
        ...
        if thread_ts and thread_ts != event.get("ts"):
            ...
            thread_history = get_thread_history(
                slack_client, openai_client, channel, thread_ts, bot_user_id
            )
            if thread_history:
                conversation.extend(thread_history)
                logger.info(
                    f"Retrieved {len(conversation)} messages from thread history"
                )
        ...
        slack_client.chat_postMessage(
            channel=channel, thread_ts=thread_ts, text=ai_response
        )
