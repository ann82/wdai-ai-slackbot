...
class UserRateLimiter:
    ...
    def is_rate_limited(self, user_id: str, team_id: Optional[str] = None) -> Tuple[bool, str]:
        """
        ...
        if user_id in self.user_requests:
            ...
            if current_time - window_start > timedelta(seconds=self.user_window_seconds):
        ...
        if team_id:
            if team_id in self.team_requests:
                ...
                if current_time - team_window_start > timedelta(seconds=self.team_window_seconds):
    ...
    def get_remaining_requests(self, user_id: str, team_id: Optional[str] = None) -> Dict[str, int]:
        """
        ...
        if user_id in self.user_requests:
            ...
            if current_time - window_start <= timedelta(seconds=self.user_window_seconds):
                ...
                result["user_window_reset_seconds"] = max(0, self.user_window_seconds - seconds_elapsed)
        ...
        if team_id and team_id in self.team_requests:
            ...
            if current_time - team_window_start <= timedelta(seconds=self.team_window_seconds):
                ...
                result["team_window_reset_seconds"] = max(0, self.team_window_seconds - seconds_elapsed)
    ...
    def clear_expired_entries(self):
        ...
        for team_id, (_, window_start) in self.team_requests.items():
            if current_time - window_start > timedelta(seconds=self.team_window_seconds):
